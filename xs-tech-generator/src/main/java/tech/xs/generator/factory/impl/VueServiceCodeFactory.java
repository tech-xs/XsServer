 ...
 */
@Component
public class VueServiceCodeFactory extends AbstractCodeFactory implements CodeFactory {

    @Resource
    protected FreeMarkerConfigurer freeMarkerConfigurer;

    @Value("${generator.vue-service.out-path:}")
    private String outPath;

    @Value("${generator.template-encoding:UTF-8}")
    private String templateEncoding;

    @Value("${generator.vue-service.template-path:freemarker/generator/VueService.ftl}")
    private String templatePath;

    @Override
    public void generator(GeneratorTable table, List<GeneratorColumn> columns) {
        String fileName = StringUtils.underlineToCamel(table.getName(), false) + FileConstant.FILE_SUFFIX_JS;
